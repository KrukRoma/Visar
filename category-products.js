...
async function loadCategoryProducts() {
    ...
    if (hasInvalidParams) return showCategoryNotFound();
    ...
    try {
        ...
        if (categoryId || subcategoryId) {
            ...
            if (categoryId) {
                if (isNaN(categoryId)) return showCategoryNotFound();

                const category = categories.find(cat => cat.CategoryID === parseInt(categoryId));
                if (!category) return showCategoryNotFound();
                ...
                if (isNaN(subcategoryId)) return showCategoryNotFound();

                let subcategory = null;
                for (const category of categories) {
                    if (Array.isArray(category.Subcategories)) {
                        subcategory = category.Subcategories.find(sub => sub.SubcategoryID === parseInt(subcategoryId));
                        if (subcategory) break;
                    }
                }

                if (!subcategory) return showSubcategoryNotFound();
            ...
            if (product.variants && product.variants.length > 0) {
                ...
                if (prices.length > 0) {
                    ...
                    price.textContent = (minPrice === maxPrice)
                        ? `${minPrice.toLocaleString('uk-UA', { minimumFractionDigits: 2 })} ₴`
                        : `${minPrice.toLocaleString('uk-UA', { minimumFractionDigits: 2 })} – ${maxPrice.toLocaleString('uk-UA', { minimumFractionDigits: 2 })} ₴`;
                } else {
    ...
    function showSubcategoryNotFound() {
        loader.style.display = "none";
        skeletonTitle.style.display = "none";
